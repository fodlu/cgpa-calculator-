...
calculateButton.addEventListener("click", function (e) {
  ...
  document.querySelectorAll("form").forEach((semester, ind) => {
    ...
    console.log(
      `Semester ${
        ind + 1
      } weight is ${semesterWeight} weights and unit is ${semesterUnit} units`
    );
    allUnitsSum += semesterUnit;
    allWeightsSum += semesterWeight;
    count++; //next semester
    let weight_container = document.querySelector(
      `#semester-weight-${ind + 1}`
    );
